...
class Service {
  ...
  create (body, params = {}) {
    ...
    let { id, uri, buffer, contentType, size } = body;
    const { sourceReadStream } = params;
    ...
    if (!uri && (!buffer || !contentType) && (!sourceReadStream || !contentType)) {
      throw new errors.BadRequest('Buffer, URI or sourceReadStream with valid content type must be provided to create a blob');
    }
    ...
    const source = sourceReadStream || fromBuffer(buffer);
    return new Promise((resolve, reject) => {
      source.pipe(this.Model.createWriteStream({
          key: id,
          params: params.s3
        }, (error) =>
          error
