...
class Service {
  ...
  create (body, params = {}) {
    ...
    if (!uri && (!buffer || !contentType) && (!path || !contentType)) {
      throw new errors.BadRequest('Buffer, path or URI with valid content type must be provided to create a blob');
    }
    ...
    const readSource = function () {
      if (buffer) return fromBuffer(buffer)
